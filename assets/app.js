...
async function renderPipeline(activeType = "investor", movedPartnerId = null) {
  ...
  if (pipelineBoardEl) {
    ...
    pipelineBoardEl.addEventListener("mousedown", (e) => {
      if (e.target.closest(".pipeline-card") || e.target.closest("button") || e.target.closest("a")) return;
      isBoardDragging = true;
      startX = e.pageX - pipelineBoardEl.offsetLeft;
      initialScrollLeft = pipelineBoardEl.scrollLeft;
    });

    const stopBoardDrag = () => { isBoardDragging = false; };
    window.addEventListener("mouseleave", stopBoardDrag);
    window.addEventListener("mouseup", stopBoardDrag);

    pipelineBoardEl.addEventListener("mousemove", (e) => {
      if (!isBoardDragging) return;
      e.preventDefault();
      const x = e.pageX - pipelineBoardEl.offsetLeft;
