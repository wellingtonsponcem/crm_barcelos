...
async function renderPipeline(activeType = "investor", movedPartnerId = null) {
  ...
  app.innerHTML = `<div style="display:flex;flex-direction:column;min-height:calc(100vh - 130px);position:relative;width:100%;max-width:100%"><div class="page-header" style="margin-bottom:12px;flex-shrink:0"><div class="page-title-group"><h2>Funil Tático de Parcerias</h2><p>Arraste visualmente os parceiros e acompanhe as negociações por etapa comercial.</p></div><div class="page-actions"><button class="btn btn-primary" id="newPipelinePartner"><span class="material-symbols-outlined">add</span>Nova Oportunidade</button></div></div><div class="card" style="margin-bottom:12px;padding:10px 16px;flex-shrink:0;background-color:var(--surface-container-low)"><div style="display:flex;gap:16px;align-items:center;flex-wrap:wrap;justify-content:space-between"><div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap"><span style="font-size:11px;font-weight:bold;text-transform:uppercase;color:var(--primary);letter-spacing:.05em">Funil Ativo:</span>${pipelineTypeButton(activeType, "investor", "Captação de Investidores")}${pipelineTypeButton(activeType, "operator", "Avaliação de Sócios/Operadores")}${pipelineTypeButton(activeType, "landowner", "Expansão de Terrenos")}</div><div style="display:flex;gap:8px;align-items:center"><span style="font-size:11px;color:var(--on-surface-variant);font-weight:bold">Rolagem Rápida:</span><button class="btn btn-secondary" id="scrollBoardLeft" style="padding:4px 10px;font-size:11px" title="Rolar para esquerda"><span class="material-symbols-outlined" style="font-size:16px">chevron_left</span> Anterior</button><button class="btn btn-secondary" id="scrollBoardRight" style="padding:4px 10px;font-size:11px" title="Rolar para direita">Próxima <span class="material-symbols-outlined" style="font-size:16px">chevron_right</span></button></div></div></div><div class="pipeline-board" style="display:flex;gap:16px;overflow-x:scroll;overflow-y:hidden;height:calc(100vh - 300px);min-height:420px;padding:0 4px 16px;margin-bottom:12px;align-items:stretch">${stages.map((s, i) => pipelineColumn(s, grouped[s], i, stages.length, movedPartnerId)).join("")}<div style="flex:0 0 16px;width:16px"></div></div><footer style="height:44px;background-color:var(--surface-container-lowest);border:1px solid var(--outline-variant);border-radius:6px;display:flex;align-items:center;padding:0 24px;justify-content:space-between;font-size:11px;flex-shrink:0"><div style="display:flex;gap:24px"><div><span style="color:var(--on-surface-variant);font-weight:bold;text-transform:uppercase;margin-right:6px">Valor Total do Funil:</span><strong style="color:var(--primary);font-family:var(--font-mono)">${money(total)}</strong></div><div><span style="color:var(--on-surface-variant);font-weight:bold;text-transform:uppercase;margin-right:6px">Ciclo Médio:</span><strong style="color:var(--primary);font-family:var(--font-mono)">24 dias</strong></div><div><span style="color:var(--on-surface-variant);font-weight:bold;text-transform:uppercase;margin-right:6px">Taxa de Conversão:</span><strong style="color:var(--primary);font-family:var(--font-mono)">68%</strong></div></div><div style="display:flex;gap:16px"><span>Ação Imediata</span><span>Alta Prioridade</span></div></footer></div>`;
  ...
  if (pipelineBoardEl) {
    ...
    $("#scrollBoardLeft")?.addEventListener("click", () => {
      pipelineBoardEl.scrollBy({ left: -340, behavior: "smooth" });
    });
